...
module.exports = {
  up: (queryInterface, Sequelize) => {
    ...
    queryInterface.sequelize.query(
      `CREATE TABLE IF NOT EXISTS "Address" ("address_id"  SERIAL , "last_name" VARCHAR(255), "first_name" VARCHAR(255), "address" VARCHAR(255) NOT NULL, "address_post" VARCHAR(255), "address_pre" VARCHAR(255), "city" VARCHAR(255) NOT NULL, "phone" VARCHAR(255), "postal_code" VARCHAR(255) NOT NULL, "country" VARCHAR(255) NOT NULL, "company" VARCHAR(255), "type_address" VARCHAR(255), "address_equal" BOOLEAN, "aboweb_address_id" VARCHAR(255), "created_at" TIMESTAMP WITH TIME ZONE NOT NULL, "updated_at" TIMESTAMP WITH TIME ZONE NOT NULL, "deleted_at" TIMESTAMP WITH TIME ZONE, "client_id" INTEGER NOT NULL REFERENCES "Client" ("client_id") ON DELETE NO ACTION ON UPDATE CASCADE, PRIMARY KEY ("address_id"));`
    )
    queryInterface.sequelize.query(
    ...
    queryInterface.sequelize.query(
      `SELECT i.relname AS name, ix.indisprimary AS primary, ix.indisunique AS unique, ix.indkey AS indkey, array_agg(a.attnum) as column_indexes, array_agg(a.attname) AS column_names, pg_get_indexdef(ix.indexrelid) AS definition FROM pg_class t, pg_class i, pg_index ix, pg_attribute a WHERE t.oid = ix.indrelid AND i.oid = ix.indexrelid AND a.attrelid = t.oid AND t.relkind = 'r' and t.relname = 'Cron' GROUP BY i.relname, ix.indexrelid, ix.indisprimary, ix.indisunique, ix.indkey ORDER BY i.relname;`
    )
    queryInterface.sequelize.query(
    ...
    queryInterface.sequelize.query(
      `CREATE TABLE IF NOT EXISTS "Subscription" ("subscription_id"  SERIAL , "aboweb_subscription_id" INTEGER, "aboweb_client_id" INTEGER, "first_number_delivered" INTEGER, "last_number_delivered" INTEGER, "is_invoiced" BOOLEAN, "is_suspended" BOOLEAN, "is_resubscription" BOOLEAN, "free_subscription" BOOLEAN, "number_of_copy" INTEGER, "order_number" VARCHAR(255), "created_at" TIMESTAMP WITH TIME ZONE NOT NULL, "updated_at" TIMESTAMP WITH TIME ZONE NOT NULL, "deleted_at" TIMESTAMP WITH TIME ZONE, PRIMARY KEY ("subscription_id"));`
    )
    queryInterface.sequelize.query(
